...
### Where does `tenant_id` originate — Azure Entra ID or local .NET code?

**It comes from the local .NET database, NOT from Azure Entra ID.**

Entra ID only provides identity information (`oid`, `email`, `name`). It has no knowledge of your application's tenants.

For Entra ID login, the flow is:

```
Entra ID provides:   oid = "abc-123"    (Microsoft user identity)
                     email, name

.NET code then:      looks up user WHERE EntraIdObjectId == "abc-123"
                     finds that user belongs to TenantId = "e5f6..."  (from AppUsers table)
                     embeds TenantId into the TeamForge JWT
```

```csharp
// AuthService.cs — EntraLoginAsync (line 236-253)
var user = await _db.AppUsers
    .Include(u => u.Tenant)
    .FirstOrDefaultAsync(u => u.EntraIdObjectId == entraUser.ObjectId);

// TenantId comes from the local database, not from Entra
return GenerateAuthResponse(user.Id, user.TenantId, ...);
```

For email/password login it's the same idea — look up the user in the local database, read their `TenantId` from the `AppUsers` table, and embed it in the JWT.

When a user registers (`AuthService.RegisterAsync`), the code creates a new `Tenant` row with a GUID generated by EF Core, and assigns that `tenant.Id` to all related entities (roles, user, branding).

### Where is `sp_set_session_context` defined?

**It is a built-in SQL Server system stored procedure.** It is not defined anywhere in the application code — it ships with SQL Server itself.

It stores a key-value pair in the current database connection's session memory:

```csharp
// TeamForgeDbContext.cs:141-143
await Database.ExecuteSqlRawAsync(
    "EXEC sp_set_session_context @key = N'TenantId', @value = {0}",
    new object[] { tenantId });
```

This value lives only for the duration of that database connection. Every HTTP request gets its own connection with its own session context — there is no cross-request leakage.

### Without specifying tenant ID in queries, how does SQL Server know which rows to filter?

SQL Server's RLS **automatically rewrites every query** behind the scenes. The SECURITY POLICY attaches the predicate function as a FILTER on each table:
...
The predicate function reads `SESSION_CONTEXT` — the value set by the middleware before any query runs:
...
So when the application code runs a query with no tenant filter:

```csharp
// Application code — no WHERE clause for tenant
var projects = await _db.Projects.ToListAsync();
```

SQL Server internally executes it as if it were:

```sql
-- What SQL Server actually runs behind the scenes
SELECT * FROM Projects
WHERE TenantId = CAST(SESSION_CONTEXT(N'TenantId') AS UNIQUEIDENTIFIER)
```

This invisible filter is injected on every SELECT, INSERT, UPDATE, and DELETE. The application code never needs to specify tenant filtering — SQL Server handles it transparently.

### Full chain per request (summary)

```
JWT claim "tenant_id"
    → TenantResolutionMiddleware extracts it from the authenticated user's claims
    → sp_set_session_context stores it in the DB connection's session memory
    → RLS predicate function reads SESSION_CONTEXT on every query
    → Only rows matching the current tenant are visible and writable
```
